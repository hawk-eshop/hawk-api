...
import { ApiKeyEntity } from '@shared/common/api-key/repository/entities/api-key.entity'
...
export class ApiKeyXApiKeyStrategy extends PassportStrategy(
  ...
  async validate(
    ...
    if (xApiKeyArr.length !== 2) {
      verified(
        new Error(`${ENUM_API_KEY_STATUS_CODE_ERROR.X_API_KEY_INVALID_ERROR}`),
        null,
        null
      )
    ...
    if (!apiKey) {
      ...
      verified(
        new Error(`${ENUM_API_KEY_STATUS_CODE_ERROR.X_API_KEY_INACTIVE_ERROR}`),
        null,
        null
      )
