...
import { AuthJwtAccessPayloadPermissionDto } from '@shared/common/auth/dtos/jwt/auth.jwt.access-payload.dto'
import {
  ENUM_POLICY_ACTION,
  ...
  handlerAbilities(
    ...
    return abilities
      .map(({ subject, action }) => {
        return action
          .map(
            (val) => (ability: IPolicyAbilityRule) => ability.can(val, subject)
          )
          .flat(1)
      })
      .flat(1)
