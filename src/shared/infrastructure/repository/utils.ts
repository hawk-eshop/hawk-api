...
export const validateFindByCommandsFilter = <T>(filterList: DatabaseOperationCommand<T>[]): void => {
  const groupList = CollectionUtil.groupBy(filterList, 'property')

  for (const key in groupList) {
    const commands = groupList[key].map((g) => g.command as DatabaseOperationEnum)
